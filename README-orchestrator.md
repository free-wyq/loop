...
```bash
loop --cwd <项目> "目标"          # 裸跑=--watch，自驱跑到完成
loop --cwd <项目> --watch "目标"  # 显式自驱
loop --cwd <项目> --tick          # 单步（幂等可恢复，给外部调度器用）
loop --cwd <项目> --status        # 实时状态（读 state.json + events.jsonl）
loop --cwd <项目> --report        # 运行报告
loop --cwd <项目> --stop          # 停（写 .stop 哨兵 + 杀 --watch）
loop --cwd <项目> --resume        # 清 .stop 哨兵恢复
loop-tick <项目>                  # 定时器 wrapper（调度器通用）
```
